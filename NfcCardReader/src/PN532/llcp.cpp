...
int8_t LLCP::waitForConnection(uint16_t timeout)
{
    ...
    // Get CONNECT PDU
    DMSG("LLCP: wait for a CONNECT PDU\n");
    do
    {
    ...
    // Put CC PDU
    DMSG("LLCP: put a CC(Connection Complete) PDU to response the CONNECT PDU\n");
...
int8_t LLCP::waitForDisconnection(uint16_t timeout)
{
    ...
    // Put DM PDU
    DMSG("LLCP: put a DM(Disconnect Mode) PDU to response the DISC PDU\n");
    // ssap = getDSAP(headerBuf);
    // dsap = getSSAP(headerBuf);
